...
const AY_DAC_TABLE = [
  0.0, 0.0, 0.00999465934234, 0.00999465934234, 0.0144502937362,
  0.0144502937362, 0.0210574502174, 0.0210574502174, 0.0307011520562,
  0.0307011520562, 0.0455481803616, 0.0455481803616, 0.0644998855573,
  0.0644998855573, 0.107362478065, 0.107362478065, 0.126588845655,
  0.126588845655, 0.20498970016, 0.20498970016, 0.292210269322, 0.292210269322,
...
const YM_DAC_TABLE = [
  0.0, 0.0, 0.00465400167849, 0.00772106507973, 0.0109559777218,
  0.0139620050355, 0.0169985503929, 0.0200198367285, 0.024368657969,
  0.029694056611, 0.0350652323186, 0.0403906309606, 0.0485389486534,
  0.0583352407111, 0.0680552376593, 0.0777752346075, 0.0925154497597,
  0.111085679408, 0.129747463188, 0.148485542077, 0.17666895552, 0.211551079576,
  0.246387426566, 0.281101701381, 0.333730067903, 0.400427252613,
  0.467383840696, 0.53443198291, 0.635172045472, 0.75800717174, 0.879926756695,
  1.0,
];
...
Ayumi.prototype.getChannels = function () {
  return [new this.Channel(), new this.Channel(), new this.Channel()];
};
...
Ayumi.prototype.updateMixer = function () {
  ...
  for (var i = 0; i < this.channels.length; i++) {
    out =
      (this.updateTone(i) | this.channels[i].tOff) &
...
// In your worklet processor file (e.g., sine-processor.js)
class AY_3_8910_Processor extends AudioWorkletProcessor {
  constructor() {
    super();

    this.ay3 = new Ayumi();
    
    this.ay3.configure(false, (3.579545 * 1000000) / 2, sampleRate);
    this.ay3.setPan(0, 0.5, false);
    this.ay3.setPan(1, 0.5, false);
    this.ay3.setPan(2, 0.5, false);

    this.port.onmessage = (event) => {
      if (
        event.data &&
        Array.isArray(event.data.psgRegisters) &&
        event.data.psgRegisters.length === 16
      ) {
        const r = event.data.psgRegisters;
        this.ay3.setTone(0, (r[1] << 8) | r[0]);
        this.ay3.setTone(1, (r[3] << 8) | r[2]);
        this.ay3.setTone(2, (r[5] << 8) | r[4]);
        this.ay3.setNoise(r[6]);
        this.ay3.setMixer(0, r[7] & 1, (r[7] >> 3) & 1, r[8] >> 4);
        this.ay3.setMixer(1, (r[7] >> 1) & 1, (r[7] >> 4) & 1, r[9] >> 4);
        this.ay3.setMixer(2, (r[7] >> 2) & 1, (r[7] >> 5) & 1, r[10] >> 4);
        this.ay3.setVolume(0, r[8] & 0xf);
        this.ay3.setVolume(1, r[9] & 0xf);
        this.ay3.setVolume(2, r[10] & 0xf);
        this.ay3.setEnvelope((r[12] << 8) | r[11]);
        if (r[13] != 255) {
          this.ay3.setEnvelopeShape(r[13]);
        }
      }
    };
  }
...
registerProcessor("AY-3-8910-processor", AY_3_8910_Processor);
