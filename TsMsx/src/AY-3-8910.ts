...
/* Javascript version: Alexander Kovalenko (https://github.com/alexanderk23/ayumi-js) */
...
export class AY_3_8910 {
  ...
  envelopes = this.getEnvelopeShapes();
  envelopeCounter = 0;
  envelopePeriod = 0;
  envelopeShape = 0;
  envelopeSegment = 0;
  ...
  firLeft = new Float64Array(FIR_SIZE * 2);
  firRight = new Float64Array(FIR_SIZE * 2);
  firIndex = 0;
  ...
  resetSegment() {
    ...
    this.envelope = (env == slideDown || holdTop) ? 31 : 0;
  }
  ...
  configure(isYM: boolean, clockRate: number, sr: number) {
  ...
  setPan(index: number, pan: number, isEqp: boolean) {
    if (isEqp) {
  ...
  setTone(index: number, period: number) {
    period &= 0xfff;
    this.channels[index].tonePeriod = (period == 0) ? 1 : period;
  }

  setNoise(period: number) {
    this.noisePeriod = period & 0x1f;
  }

  setMixer(index: number, tOff: number, nOff: number, eOn: number) {
  ...
  setEnvelope(period: number) {
    period &= 0xffff;
    this.envelopePeriod = period == 0 ? 1 : period;
  }

  setEnvelopeShape(shape: number) {
  ...
  dcFilter(dc: Filter, index: number, x: number) {
